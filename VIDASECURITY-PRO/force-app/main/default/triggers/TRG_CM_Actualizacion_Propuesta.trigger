...
**/
trigger TRG_CM_Actualizacion_Propuesta on  Opportunity (after update, after delete) {
    if(Trigger.isAfter){
        if(Trigger.isUpdate){
            try{
                ...
                if( op.StageName == 'Venta - Aprobada' && op.OPOR_WS_Actualiza_Datos_Prospecto__c == false ){
                    ...
                    //Llamada a WS Onco
                   EXT_WS_CM_Oncologico.actualizarPropuesta(usuario, op.Id); 

                   Boolean validacionConsulta = [SELECT OPOR_WS_Actualiza_Datos_Prospecto__c  FROM Opportunity WHERE Id =: op.Id].OPOR_WS_Actualiza_Datos_Prospecto__c;
                   System.debug('@@@@ validacionConsulta => ' + validacionConsulta);
                   if (validacionConsulta == false)
                   {
                        try
                        {
                            BAT_CM_Validacion_Propuesta j_MConco = new BAT_CM_Validacion_Propuesta(op.Id);     
